...
datasetFrame = cv2.imread("example.png")
...
for rect in datasetRects:
	...
	if len(shape):
		(x_min, y_min) = (x_max, y_max) = shape[0]
	...
	for (x, y) in shape: 
		...
		if y > y_max :
			y_max = y
		cv2.circle(datasetFrame, (x, y), 1, (0, 0, 255), -1)

cv2.rectangle(datasetFrame, (x_min, y_min), (x_max, y_max), (0, 255, 0), 3)	
cv2.imshow("Example", datasetFrame)
